...
function updateScrollAnimation() {
	...
	teams.forEach((team, idx) => {
		...
		new ScrollMagic.Scene({
			...
			triggerHook: 0.5,
			duration: duration,
		}).setClassToggle(id, "visible").addTo(scrollController);
	})
	new ScrollMagic.Scene({
		...
		duration: "80%",
		offset: 20
	}).setClassToggle("#links", "visible").addTo(scrollController);
}
onMounted(() => {
...
<template>
	<div class="prose mx-auto mt-10 w-full pb-60 font-mono" id="links">
		...
		<div class="mt-16 flex w-full flex-col items-center">
			...
			<div v-for="(team, index) in teams" class="flex w-full flex-col items-center reveal" :ref="setTeamRefs"
